...
function updateReadingTime(content, readingTime) {
    ...
    return content.replace(/(^---[\s\S]*?)(---)/, `$1readingTime: "${readingTime}"\n$2`)
}
...
function main() {
    ...
    for (const file of files) {
        ...
        for (const field of REQUIRED_FIELDS) {
            if (!frontmatter || frontmatter[field] === undefined || frontmatter[field] === '') {
    ...
    if (updatedCount > 0) {
        console.log(`✨ Updated ${updatedCount} file(s). Don't forget to stage them!`)
    } else {

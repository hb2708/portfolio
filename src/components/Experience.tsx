            ...
            <div className="container mx-auto px-6 relative z-10" ref={containerRef}>
                ...
                >
                    <h2 className="text-4xl md:text-5xl font-bold text-text mb-12 flex items-center">
                        <span className="text-primary mr-2">{EXPERIENCE_CONTENT.sectionTitle.number}</span> {EXPERIENCE_CONTENT.sectionTitle.text}
                    </h2>
                </motion.div>
                    ...
                    {EXPERIENCES.map((exp, index) => (
                        ...
                        >
                            ...
                            <div className="hidden md:block absolute left-0 top-[-5px] w-[100px] text-right">
                                <span className="font-mono text-sm text-muted group-hover:text-primary transition-colors">{exp.period}</span>
                            </div>
                            ...
                            <div className="relative">
                                ...
                                <h4 className="text-lg text-muted mb-6 font-mono">{exp.role}</h4>

                                <ul className="space-y-3 mb-6">
                                    {exp.description.map((desc, idx) => (
                                        <li key={idx} className="text-muted text-base leading-relaxed flex items-start">
                                ...
                                <div className="flex flex-wrap gap-2">
                                    {exp.tech.map((tech, idx) => (
                                        <span key={idx} className="text-xs font-mono text-primary/80 bg-primary/5 px-2 py-1 border border-primary/20 uppercase tracking-wider">
                                            {tech}
                                        </span>
                                    ))}
                                </div>

            ...
            <div className="container mx-auto px-6 relative z-10">
                ...
                >
                    <h2 className="text-4xl md:text-5xl font-bold text-text mb-12 flex items-center">
                        <span className="text-primary mr-2">{PROJECTS_CONTENT.sectionTitle.number}</span> {PROJECTS_CONTENT.sectionTitle.text}
                    </h2>
                </motion.div>
                ...
                >
                    {PROJECTS.map((project, index) => (
                        <motion.div
                            ...
                            className={`group relative bg-surface border border-white/5 hover:border-primary/30 transition-all duration-300 overflow-hidden flex flex-col rounded-xl ${index === 0 || index === 3 ? 'md:col-span-2' : ''
                                }`}
                        >
                            {/* Tech Header */}
                            ...
                            <div className="p-6 md:p-8 flex flex-col flex-grow border-t border-white/5 bg-surface relative">
                                ...
                                <div className="flex flex-wrap gap-2 mb-8 mt-auto">
                                    {project.tech.slice(0, 4).map((t) => (
                                        <span key={t} className="text-xs font-mono text-primary/80 bg-primary/5 px-2 py-1 border border-primary/20 uppercase tracking-wider">
                                            {t}
                                        </span>
                                    ))}
                                </div>
                                ...
                                {searchParams.get('debug') === 'true' ? (
                                    ...
                                    <div className="flex flex-wrap gap-4">
                                        {project.links ? (
                                            <>
                                                {project.links.ios && (
                                                    <a href={project.links.ios} target="_blank" rel="noopener noreferrer" aria-label={`View ${project.title} on App Store`} className="flex items-center justify-center w-10 h-10 rounded-full bg-white/5 border border-white/10 text-muted hover:bg-primary/10 hover:border-primary/50 hover:text-primary transition-all duration-300 hover:scale-110">
                                                        <AppStoreIcon className="w-5 h-5" />
                                                    </a>
                                                )}
                                                {project.links.android && (
                                                    <a href={project.links.android} target="_blank" rel="noopener noreferrer" aria-label={`View ${project.title} on Google Play Store`} className="flex items-center justify-center w-10 h-10 rounded-full bg-white/5 border border-white/10 text-muted hover:bg-primary/10 hover:border-primary/50 hover:text-primary transition-all duration-300 hover:scale-110">
                                                        <PlayStoreIcon className="w-5 h-5" />
                                                    </a>
                                                )}
                                                {project.links.web && (
                                                    <a href={project.links.web} target="_blank" rel="noopener noreferrer" aria-label={`View ${project.title} website`} className="flex items-center justify-center w-10 h-10 rounded-full bg-white/5 border border-white/10 text-muted hover:bg-primary/10 hover:border-primary/50 hover:text-primary transition-all duration-300 hover:scale-110">
